...
const SPREADSHEET_ID = '1U27WZTOkEAfNiVmQd02jGswnlZHGxLZn8aFWpPTuMug';
...
function calculateGrossMargin(projectId) {
  try {
    return calcGrossMarginValue(projectId);
  } catch (e) {
    return null;
  }
}

function calcGrossMarginValue(projectId) {
  const billings = getBillings(projectId);
  if (billings.error) return null;
  const billingTotal = billings.reduce((s, b) => s + (Number(b['請求金額']) || 0), 0);
  if (!billingTotal || billingTotal === 0) return null;
  const expenses = getExpenses(projectId);
  if (expenses.error) return null;
  return _calcMarginFromExpenses(projectId, billingTotal, expenses);
}

/** 読み込み済みの経費・請求配列から粗利率を計算（シート再読み込みなし）
 *  粗利率 = 100 − (経費合計 ÷ 請求合計 × 100)
 */
function _calcMarginFromExpenses(projectId, billingTotal, allExpenses) {
  if (!billingTotal || billingTotal === 0) return null;
  const totalExpenses = allExpenses
    .filter(e => String(e['台帳番号']) === String(projectId))
    .reduce((sum, e) => sum + (Number(e['金額']) || 0), 0);
  const margin = 100 - (totalExpenses / billingTotal * 100);
 ...
 */
function getProjects(preloadedExpenses, preloadedBillings) {
  try {
    const projects    = getSheetData(SHEETS.PROJECTS);
    const allExpenses = preloadedExpenses || getSheetData(SHEETS.EXPENSES);
    const allBillings = preloadedBillings || getSheetData(SHEETS.BILLINGS);
    return projects.map(p => {
      const pid = String(p['台帳番号']);
      const billingTotal = allBillings
        .filter(b => String(b['台帳番号']) === pid)
        .reduce((s, b) => s + (Number(b['請求金額']) || 0), 0);
      const margin = _calcMarginFromExpenses(pid, billingTotal, allExpenses);
      return {
        ...p,
...
function getProjectById(projectId) {
  try {
    ...
    const billings = getBillings(projectId);
    const margin = calcGrossMarginValue(String(projectId));
...
function getDashboardData() {
  try {
    ...
    const projects    = getProjects(allExpenses, allBillings); // 読み込み済み経費・請求を渡して二重読み込みを防ぐ
...
function getGanttData() {
  try {
    ...
    const rows = projects.map(p => {
      ...
      // 粗利率 = 100 − (経費合計 ÷ 請求合計 × 100)
      const margin = billingTotal > 0 ? Math.round((100 - expTotal / billingTotal * 100) * 10) / 10 : null;
      return {
        台帳番号:   p['台帳番号'],
        案件名:     p['案件名'],
        客先名:     p['客先名'],
